...
_ACTIVE_CAPTURE: contextvars.ContextVar[WeightLoadCapture | None] = (
    contextvars.ContextVar("sglang_weight_load_capture", default=None)
)
...
class WeightLoadRecorder:
    ...
    def record_model_load(
        ...
        *,
        ...
        with self._wrap_parameter_loaders(
            ...
            try:
                with _WeightLoadDispatchMode(
                    ...
                    result = model.load_weights(tracked_weights)
            finally:
...
class WeightLoadCapture:
    ...
    def record_model_load(self, model: Any, weights: Any) -> Any:
        return self._recorder.record_model_load(
            model, weights, execute_writes=True
        )
...
@contextlib.contextmanager
def capture_weight_load_plan() -> Iterator[WeightLoadCapture]:
...
def maybe_record_model_weight_load(model: Any, weights: Any) -> Any:
    capture = _ACTIVE_CAPTURE.get()
    if capture is None:
        return model.load_weights(weights)
    return capture.record_model_load(model, weights)
